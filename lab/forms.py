...
class JobStatus(forms.Form):
    ...
    """
    name = forms.CharField(max_length=5, label='Team member name', widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Team member'}),)
    email = forms.EmailField(required=True, widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'email'}))
    job_name = forms.CharField(max_length=30, label='Job name', widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Job name'}))
